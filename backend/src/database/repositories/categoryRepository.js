...
class CategoryRepository {
  ...
  async findById(id, options) {
    return MongooseRepository.wrapWithSessionIfExists(
      Category.findById(id).populate('questionnaires'),
      options,
    );
  }
  ...
  async findAndCountAll(
    ...
    const rows = await Category.find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);
    const count = await Category.countDocuments(criteria);
  ...
  async findAllAutocomplete(search, limit) {
    ...
    const records = await Category.find(criteria)
      .limit(limitEscaped)
      .sort(sort);
